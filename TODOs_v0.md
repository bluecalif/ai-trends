...
### 1.9 API 엔드포인트
- [x] `backend/app/api/__init__.py` 생성 (이미 존재)
- [x] `backend/app/api/sources.py` 생성:
  - [x] `GET /api/sources` - 소스 목록
  - [x] `POST /api/sources` - 소스 추가
  - [x] `GET /api/sources/{id}` - 소스 상세
  - [x] `PUT /api/sources/{id}` - 소스 수정
  - [x] `DELETE /api/sources/{id}` - 소스 삭제
- [x] `backend/app/api/items.py` 생성:
  - [x] `GET /api/items` - 아이템 목록 (필터: custom_tag, 날짜, source_id, 페이지네이션, 정렬)
  - [x] `GET /api/items/{id}` - 아이템 상세
  - [x] `GET /api/items/group/{dup_group_id}` - 사건 타임라인
- [x] `backend/app/api/groups.py` 생성 (이미 존재):
  - [x] `GET /api/groups` - `since`, `kind=new|incremental` 기반 그룹 목록(대표+updates_count_since)
  - [x] `GET /api/groups/{dup_group_id}` - 그룹 타임라인 상세
- [x] `backend/app/api/persons.py` 생성:
  - [x] `GET /api/persons` - 인물 목록
  - [x] `GET /api/persons/{id}` - 인물 상세 (타임라인+그래프)
  - [x] `POST /api/persons` - 인물 추가
- [x] `backend/app/api/bookmarks.py` 생성:
  - [x] `GET /api/bookmarks` - 북마크 목록 (태그 필터링)
  - [x] `POST /api/bookmarks` - 북마크 추가 (item_id 또는 link 기반)
  - [x] `PUT /api/bookmarks/{id}` - 북마크 수정
  - [x] `DELETE /api/bookmarks/{id}` - 북마크 삭제
- [x] `backend/app/api/watch_rules.py` 생성:
  - [x] `GET /api/watch-rules` - 워치 규칙 목록 (person_id 필터링)
  - [x] `POST /api/watch-rules` - 워치 규칙 추가
  - [x] `GET /api/watch-rules/{id}` - 워치 규칙 상세
  - [x] `PUT /api/watch-rules/{id}` - 워치 규칙 수정
  - [x] `DELETE /api/watch-rules/{id}` - 워치 규칙 삭제
- [x] `backend/app/api/insights.py` 생성:
  - [x] `GET /api/insights/weekly` - 주간 인사이트
  - [x] `GET /api/insights/keywords` - 키워드 트렌드
  - [x] `GET /api/insights/persons/{person_id}` - 인물별 인사이트
- [x] `backend/app/main.py`에 모든 라우터 등록
- [x] Pydantic 스키마 작성 (`backend/app/schemas/`)
  - [x] `schemas/item.py` - ItemResponse, ItemListResponse
  - [x] `schemas/source.py` - SourceResponse, SourceCreate, SourceUpdate
  - [x] `schemas/person.py` - PersonResponse, PersonCreate, PersonDetailResponse, PersonTimelineEventResponse
  - [x] `schemas/bookmark.py` - BookmarkResponse, BookmarkCreate, BookmarkUpdate
  - [x] `schemas/watch_rule.py` - WatchRuleResponse, WatchRuleCreate, WatchRuleUpdate
  - [x] `schemas/insights.py` - WeeklyInsightResponse, KeywordTrendResponse, PersonInsightResponse
- [x] `backend/app/core/constants.py`에 FIELDS, CUSTOM_TAGS 추가
- [x] **E2E 테스트**: 전체 API 플로우 테스트 완료 (`test_api_e2e_full_flow`)
  - [x] Items API 테스트 (1177개 아이템, 필터링, 페이지네이션)
  - [x] Sources API 테스트 (10개 소스)
  - [x] Persons API 테스트 (5개 인물, 타임라인, 관계 그래프)
  - [x] Bookmarks API 테스트 (생성/삭제)
  - [x] Watch Rules API 테스트 (6개 규칙)
  - [x] Insights API 테스트 (주간 인사이트, 키워드 트렌드, 인물별 인사이트)
- [x] PostgreSQL JSONB 배열 검색 수정 (`@>` 연산자 사용, JSON → JSONB 캐스팅)
- [x] API 문서 확인 (FastAPI 자동 생성: `/docs`)
  - [x] OpenAPI 3.1.0 스키마 검증 완료
  - [x] 총 29개 엔드포인트 확인 (20개 경로)
  - [x] 모든 API 태그 및 설명 확인
- [x] Supabase 연동 확인: 필터/페이지네이션/정렬이 Supabase에서 일관 동작
  - [x] 페이지네이션 검증 (페이지별 일관된 total, 중복 없음)
  - [x] 필터링 검증 (custom_tag, date_from 필터 정상 동작)
  - [x] 정렬 검증 (published_at 기준 오름차순/내림차순 정상 동작)
  - [x] E2E 테스트 통과 (`test_supabase_pagination_filtering_sorting`)
...
**마지막 업데이트**: 2025-11-19 (Phase 1.9 완료 ✅, Phase 1 전체 완료 ✅)

**프로젝트 진행률**: 백엔드 기반 구조 100% 완료 (Phase 1.1~1.9 완료, 모든 검증 통과 ✅)

**완료된 항목**: Phase 1.1, Phase 1.2, Phase 1.3, Phase 1.4, Phase 1.5, Phase 1.6, Phase 1.7, Phase 1.8, Phase 1.9 (서비스 구현, API 엔드포인트, E2E 테스트, API 문서, Supabase 연동 검증 완료 ✅)

**현재 단계**: Phase 1 전체 완료 ✅, Phase 2 (프론트엔드) 시작 준비 완료
